...
function TakeResult({ pass, analysis, url, openFactor, setOpenFactor }: { pass: Pass; analysis: ToneAnalysis; url: string | null; openFactor: { pass: Pass; factor: FactorKey } | null; setOpenFactor: (value: { pass: Pass; factor: FactorKey } | null) => void }) {
  ...
  return <div className="nb bg-card p-4"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold uppercase tracking-widest">{pass === "reflex" ? "Reflex take" : "Intended take"}</p><span className={cn("nb px-2 py-1 text-[10px] font-bold uppercase tracking-widest", tone.color)}>{tone.label}</span></div><p className="mt-2 text-sm text-muted-foreground">{tone.note}</p><div className="mt-3 nb bg-secondary p-3"><p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Overall rating</p><p className="font-display text-4xl">{analysis.overallScore}<span className="text-base text-muted-foreground"> / 100</span></p></div><PlaybackPanel label={pass === "reflex" ? "Reflex take" : "Intended take"} url={url} /><div className="mt-4 flex flex-col gap-2">{FACTORS.map((factor) => { const open = openFactor?.pass === pass && openFactor.factor === factor; const fb = feedback[factor]; return <div key={factor} className="nb bg-paper"><button type="button" onClick={() => setOpenFactor(open ? null : { pass, factor })} aria-expanded={open} className="flex w-full items-center justify-between px-3 py-2 text-left"><span className="text-xs font-bold uppercase tracking-widest">{TONE_FACTORS[factor].label}</span><span className="font-display text-xl">{scoreFor(factor, analysis)} / 100</span></button>{open && <div className="border-t-2 border-ink/20 px-3 py-3 text-sm"><p>{fb.read}</p><p className="mt-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">{fb.yourNumbers}</p><p className="mt-2"><span className="font-bold">Next practice: </span><span className="text-muted-foreground">{fb.tip}</span></p><p className="mt-2 text-xs text-muted-foreground"><span className="font-bold">Goal: </span>{TONE_FACTORS[factor].goal}</p></div>}</div>; })}</div><div className="mt-4 nb bg-sun p-3 text-sm"><span className="font-bold">Biggest lever: </span>{TONE_FACTORS[lever.factor].label} — {lever.tip}</div></div>;
}
